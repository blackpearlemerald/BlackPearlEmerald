...
#define OW_PC_HEAL                  GEN_7      // In Gen8+, Pokémon are not healed when deposited in the PC. BPE 2.1: PC records do not store HP, status or PP, so deposited Pokémon are always healed.
...
#define OW_SYNCHRONIZE_NATURE       GEN_8 // In Gen8+, if a Pokémon with Synchronize leads the party, wild Pokémon will always have their same Nature as opposed to the 50% chance in previous games. Gift Pokémon excluded.
...
#define OW_FLAG_NO_TRAINER_SEE      FLAG_UNUSED_0x022  // If this flag is set, trainers will not battle the player unless they're talked to.
#define OW_FLAG_NO_COLLISION        FLAG_UNUSED_0x021  // If this flag is set, the player will be able to walk over tiles with collision. Mainly intended for debugging purposes.
