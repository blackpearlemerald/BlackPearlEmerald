...
#define USE_DEXNAV_SEARCH_LEVELS      FALSE  /* WARNING: POSSIBLY EXCEEDS SAVEBLOCK SPACE! REQUIRES 1 BYTE PER SPECIES */
                                             // BPE: the save has no room for it, so the current chain is the search level (GetSearchLevel)

// Flag/var defines
// BPE: DexNav is a Standard mode tool. It comes with the Pokédex, and
// IsDexNavUnlocked() keeps it out of Nuzlocke games. BPE's wild tables have no
// hidden Pokémon, so detector mode has nothing to find.
...
#define DN_FLAG_DETECTOR_MODE         FLAG_SYS_POKEDEX_GET  // Allow player to find hidden mons
