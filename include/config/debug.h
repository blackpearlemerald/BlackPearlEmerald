...
// Overworld Debug
#define DEBUG_OVERWORLD_MENU            TRUE                // BPE: enabled in release builds, as in 1.0.1. Enables an overworld debug menu to change flags, variables, giving Pokémon and more, accessed by holding R and pressing START while in the overworld by default.
#define DEBUG_OVERWORLD_HELD_KEYS       (SELECT_BUTTON)     // BPE: SELECT + START, as in 1.0.1. The keys required to be held to open the debug menu.
...
// Battle Debug Menu
#define DEBUG_BATTLE_MENU               TRUE                // BPE: enabled in release builds, as in 1.0.1. If set to TRUE, enables a debug menu to use in battles by pressing the Select button.
