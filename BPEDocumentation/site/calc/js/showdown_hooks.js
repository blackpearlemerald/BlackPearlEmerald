...
{
    // BPE Emerald: group a trainer's full team by tr_id. This is robust against
    // BPE's many duplicate trainer names ("GRUNT"), unlike the upstream
    // display-name string parsing.
    var all_poks = SETDEX_BW
    var matches = []

    var species = trainer_name.split(" (")[0]
    var inner = trainer_name.split(" (")[1] || ""
    var set_name = inner.substring(0, inner.lastIndexOf(")"))

    var selected = all_poks[species] && all_poks[species][set_name]
    if (!selected || typeof selected.tr_id === "undefined") {
        return matches
    }
    var tr_id = selected.tr_id

    for (var sp in all_poks) {
        var sets = all_poks[sp]
        for (var key in sets) {
            if (sets[key].tr_id === tr_id) {
                var sub = sets[key].sub_index || 0
                matches.push(`${sp} (${key})[${sub}]`)
            }
        }
    }

    matches.sort(function (a, b) {
        function sub(s) { return parseInt(s.split("[").pop()) || 0 }
        return sub(a) - sub(b)
    })
    return matches
}
...
$(document).ready(function() {
   ...
   // BPE Emerald: self-hosted local data source (no npoint.io dependency)
   if (params.get('data') === 'bpe') {
    npoint = './data/bpe_calc_data.json'
   }
   ...
   if (typeof backupFiles !== 'undefined' && backupFiles[TITLE]) {
