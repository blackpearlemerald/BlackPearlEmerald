...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId)
{
    ...
    // BPE Nuzlocke: dead Pokémon can only show summary, switch, and cancel. No field moves or items.
...
void ItemUseCB_Medicine(u8 taskId, TaskFunc task)
{
    ...
    if (cannotUse != FALSE)
    {
        gPartyMenuUseExitCallback = FALSE;
        PlaySE(SE_SELECT);
        if (canHeal && FlagGet(FLAG_NUZLOCKE) && GetMonData(mon, MON_DATA_DEAD)) // BPE Nuzlocke
