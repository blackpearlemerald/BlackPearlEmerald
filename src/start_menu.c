...
static void ShowTimeWindow(void)
{
    ...
    //StringExpandPlaceholders(gStringVar4, gDayNameStringsTable[(gLocalTime.days % 7)]);  //weekday varient
    ...
    *ptr = 0xF0;
    ...
    AddTextPrinterParameterized(sStartClockWindowId, 1, gStringVar4, GetStringRightAlignXOffset(1, suffix, CLOCK_WINDOW_WIDTH) - (CLOCK_WINDOW_WIDTH - GetStringRightAlignXOffset(1, gStringVar4, CLOCK_WINDOW_WIDTH) - 18), 1, 0xFF, NULL); // print time

    AddTextPrinterParameterized(sStartClockWindowId, 1, suffix, GetStringRightAlignXOffset(1, suffix, CLOCK_WINDOW_WIDTH) + (20), 1, 0xFF, NULL); // print am/pm
...
static bool8 HandleStartMenuInput(void)
{
    ...
    if (JOY_NEW(A_BUTTON))
    {
        ...
        if (sCurrentStartMenuActions[sStartMenuCursorPos] == MENU_ACTION_DEXNAV
          && (MapHasNoEncounterData() || !IsDexNavUsableHere()))
            return FALSE;
