...
> **Black Pearl Emerald checkout:** the repository-local, read-only CLI and its exact release-profile workflow are documented in [BPE_README.md](BPE_README.md). It supports both `.sav` and raw SRAM `.srm` files without launching an emulator.
